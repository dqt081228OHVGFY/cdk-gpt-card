...
GPT 发卡网的 CDK 兑换、账号库存管理与 JSON 格式转换源码。
...
- CDK 兑换和格式转换链接下载后立即失效并删除临时文件；未下载的兑换链接最长保留 24 小时，格式转换链接最长保留 10 分钟。
...
> 本公开仓库不包含生产 `config.ini`、管理员账号或密码、CDK、账号 JSON、数据库、日志、临时下载文件及任何管理密钥。
...
- CDK 兑换链接下载后立即失效，未下载最长保留 24 小时；格式转换链接下载后立即失效，未下载最长保留 10 分钟。兑换链接过期后可在后台重新生成，不会重新分配账号。
